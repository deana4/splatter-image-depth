...
class GaussianSplatPredictor(nn.Module):
    ...
    # added the depth image as the input to the network
    def forward(self, x, depth_image,
        ...
        # convert depth images to torch
        depth_image = depth_image.to(torch.float32)
        source_cameras_view_to_world = source_cameras_view_to_world.to(torch.float32)
        ...
        if self.cfg.model.network_with_offset:
            ...
            _, offset, opacity, scaling, rotation, features_dc = split_network_outputs[:6]
            if self.cfg.model.max_sh_degree > 0:
                features_rest = split_network_outputs[6]

            pos = self.get_pos_from_network_output(depth_image, offset, focals_pixels, const_offset=const_offset)
            ...
            _, opacity, scaling, rotation, features_dc = split_network_outputs[:5]
            if self.cfg.model.max_sh_degree > 0:
                features_rest = split_network_outputs[5]

            pos = self.get_pos_from_network_output(depth_image, 0.0, focals_pixels, const_offset=const_offset)
